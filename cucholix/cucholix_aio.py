...
def process_pchtxt(root_folder, output_path):
    ...
    for current_root, dirs, files in os.walk(root_folder):
        ...
        for file in files:
            ...
            # Extract Title ID from pchtxt and create additional file
            try:
                with open(source_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if title_id_match:
                        ...
                        print(f"Created Title ID file: {title_id_file_path}")
                    else:
                        print(f"Warning: No Title ID found in {source_path}")
            except Exception as e:
                print(f"Error processing Title ID for {source_path}: {e}")
